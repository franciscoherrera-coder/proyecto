...
@section('content')
    ...
    <div class="col-lg-8 col-md-10 mx-auto p-4 border border-1 rounded shadow-sm">
        ...
        <div class="mb-3 form-check">
            {{ Form::hidden('laboratorio', 0) }} {{-- valor por defecto si no se marca --}}
            {{ Form::checkbox('laboratorio', 1, old('laboratorio'), ['class' => 'form-check-input', 'id' => 'laboratorio']) }}
            {{ Form::label('laboratorio', 'Laboratorio', ['class' => 'form-check-label']) }}
        </div>
        <button type="submit" class="btn btn-success btn-block w-100">{{ __('Guardar') }}</button>
    </div>
    {!! Form::close() !!}
@endsection